...
/***/ function(module, exports, __webpack_require__) {
	...
	var _colourUtils = __webpack_require__(305);
	
	var _shadermaterial = __webpack_require__(302);
	...
	var shades = (0, _colourUtils.generateShades)('235', 10, 80, 90).map(function (color) {
	...
	var randomShade = function randomShade() {
	...
	for (var _x = minX; _x < maxX; _x++) {
	  for (var _z = minZ; _z < maxZ; _z++) {
	    for (var y = minY; y < maxY; y++) {
	      ...
	      var color = pick(_colourUtils.flatUIHexColors);
	...
	var widthController = gui.add(p, 'width', 100, 500);
	var heightController = gui.add(p, 'height', 100, 500);
	var wSegmentsCtrl = gui.add(p, 'widthSegments', 100, 500);
	var hSegmentsCtrl = gui.add(p, 'heightSegments', 100, 500);
	var colorCtrl = gui.addColor(p, 'color');
	...
	function update() {
	  ...
	  plane.geometry.vertices.forEach(function (v) {
	    v.z = Math.sin(delta / p.period + v.x * p.sinXOffset + v.y * p.sinYOffset) * p.sinAmplitude + Math.cos(delta / p.period + v.x * p.cosXOffset + v.y * p.cosYOffset) * p.cosAmplitude + Math.tan(delta / p.period + v.x * p.tanXOffset + v.y * p.tanYOffset) * p.tanAmplitude;
	  });
	...
	var planeMaker = function planeMaker() {
	  var geom = new THREE.PlaneGeometry(p.width, p.height, p.widthSegments, p.heightSegments);
	  var mat = new THREE.MeshBasicMaterial({ color: p.color, side: THREE.DoubleSide, wireframe: p.wireframe });
	...
	wSegmentsCtrl.onChange(function () {
	  scene.remove(plane);
	  plane = planeMaker();
	  scene.add(plane);
	});
	hSegmentsCtrl.onChange(function () {
	  scene.remove(plane);
	  plane = planeMaker();
	  scene.add(plane);
	});
	...
	Object.keys(watchers).forEach(function (key) {
	  watchers[key].onChange(function () {
	    scene.remove(plane);
	    plane = planeMaker();
	    scene.add(plane);
	  });
	...
	var render = function render(ts) {
	  ...
	  update(ts);
	  _shadermaterial.uniforms.time.value += _shadermaterial.uniforms.time.step;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _stats = __webpack_require__(300);
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = function () {
	  ...
	  var plane = new THREE.Mesh(planeGeom, new THREE.MeshLambertMaterial({ side: THREE.DoubleSide }));
	
	  plane.rotation.x = Math.PI / 2;
	
	  var cube = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1), _shadermaterial.shaderMaterial
	  // new THREE.MeshNormalMaterial()
	  );
	
	  return { plane: plane, cube: cube };
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.shaderMaterial = exports.uniforms = undefined;
	
	var _vert = __webpack_require__(303);
	
	var _vert2 = _interopRequireDefault(_vert);
	
	var _frag = __webpack_require__(304);
	
	var _frag2 = _interopRequireDefault(_frag);
	
	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
	
	console.log('vert: ', _vert2.default);
	
	var uniforms = exports.uniforms = {
	  time: { type: 'f', value: 0.0, step: 0.03 }
	};
	
	var shaderMaterial = exports.shaderMaterial = new THREE.ShaderMaterial({
	  uniforms: uniforms,
...
/***/ function(module, exports) {
	...
	var blues = exports.blues = [0x3F51B5, 0x3949AB, 0x303F9F, 0x283593, 0x1A237E];
	...
	var generateShades = exports.generateShades = function generateShades(hue, numOfShades) {
	  var low = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 10;
	  var high = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 90;
